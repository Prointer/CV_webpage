...
const HardSkillCard: React.FC<HardSkillCardProps> = ({ icon, title, description }) => {
  ...
  return (
    <div
      className="bg-bgbrand-light dark:bg-bgbrand-dark/70 p-6 rounded-xl shadow-md hover:shadow-lg dark:hover:shadow-brand/40 dark:hover:shadow-md transition-all duration-300 ease-in-out group flex flex-col"
    ...
    >
      ...
      <div className="flex items-center mb-4">
        <div className="mr-4 flex-shrink-0">
          ...
          <div className="w-12 h-12 flex items-center justify-center rounded-lg bg-brand/10 dark:bg-brand-dark/20 p-1">
            {typeof icon === 'string' ? <img src={icon as string} alt={`${title} icon`} className="w-full h-full object-contain" /> : icon}
          </div>
        </div>
      ...
      <div
        ...
        className={`overflow-hidden transition-all ease-in-out duration-500 ${isExpanded ? 'max-h-96 opacity-100 visible' : 'max-h-0 opacity-0 invisible'} mb-4 flex-grow`}
      >
        <div className="pt-2"> {/* Add some padding when expanded */}
...
export const PlaceholderSkillIcon: React.FC<{bgColor?: string}> = ({bgColor = 'bg-brand'}) => (
  <div className={`w-full h-full ${bgColor} rounded-md flex items-center justify-center text-white font-bold text-xl p-1`}>
    S
  </div>
);
