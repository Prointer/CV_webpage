...
const ZigzagTimelineEvent: React.FC<ZigzagTimelineEventProps> = ({ event, alignment, onDotClick, isLast }) => {
  ...
  const contentBoxClasses = "p-4 bg-bgbrand-light dark:bg-bgbrand-dark/80 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300";
  ...
  if (isCenterAligned) {
    ...
    return (
      ...
      >
        ...
        {!isLast && <div className="w-1 h-8 bg-gray-300 dark:bg-gray-700"></div>}

        {/* Content Block - Centered below dot */}
        <div className={`mt-4 w-full max-w-md ${contentBoxClasses}`}>
          <p className="text-sm font-semibold text-brand dark:text-brand-light mb-1 text-center">{event.date}</p>
          <h4 className="text-lg font-bold text-gray-800 dark:text-white mb-2 text-center">{event.title}</h4>
          <button
            onClick={() => onDotClick(event.id)}
  ...
  return (
    ...
    >
      ...
      <div className={`w-1/2 ${isLeftAligned ? 'pr-10 text-right' : 'pl-10 text-left order-3'}`}>
        <div className={contentBoxClasses}>
          <p className="text-sm font-semibold text-brand dark:text-brand-light mb-1">{event.date}</p>
          <h4 className="text-md font-bold text-gray-800 dark:text-white mb-1">{event.title}</h4>
          <button
            onClick={() => onDotClick(event.id)}
      ...
      <div className={`w-0 relative flex-shrink-0 ${isLeftAligned ? 'order-2' : 'order-2'}`}>
      ...
      <div className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-10 ${isLeftAligned ? 'md:left-[calc(50%_-_2.5rem)]' : 'md:left-[calc(50%_+_2.5rem)]' } `}>
        ...
        <div
            className={`absolute top-1/2 -translate-y-1/2 h-0.5 bg-gray-300 dark:bg-gray-600
                        ${isLeftAligned ? 'right-[calc(100%_+_4px)] w-10' : 'left-[calc(100%_+_4px)] w-10'}`}
