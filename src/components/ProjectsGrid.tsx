		...
		<article className='glass group rounded-2xl p-4 sm:p-5 hover:shadow-xl transition hover:-translate-y-0.5'>
			...
			<div className='mt-4 flex items-start gap-2'>
				...
				{p.repoPrivate && (
					<span className='ml-2 text-[11px] px-2 py-0.5 rounded-full bg-white/10 text-white/80'>
						{t('projects.buttons.privateRepo')}
					</span>
				)}
			</div>
			...
			<div className='mt-4 flex flex-col sm:flex-row gap-2'>
				...
				{hasRepo ? (
					...
					>
						{t('projects.buttons.unavailableRepo')}
					</button>
				)}
			</div>
		</article>
...
const ProjectsGrid: React.FC<GridProps> = ({
	projects,
	className = '',
	title = 'projects.content.title',
	subtitle,
}) => {
	const { t } = useTranslation()
	return (
			...
			<div className='container mx-auto px-4'>
				<header className='mb-4'>
					<h2 className='text-2xl sm:text-3xl font-bold'>
						<span className={BRAND}>▸</span> {t(title)}
					</h2>
					{subtitle && (
